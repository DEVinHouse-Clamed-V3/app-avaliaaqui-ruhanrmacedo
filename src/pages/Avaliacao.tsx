...
export default function Avaliacao({ route, navigation }) {
    ...
    const fetchReviews = async () => {
        try {
            const response = await axios.get(`http://192.168.1.51:3000/evaluations?productId=${productId}`);
    ...
    const submitEvaluation = async () => {
        ...
        setTimeout(async () => {
            try {
                const response = await axios.post('http://192.168.1.51:3000/evaluations', {
                    productId,
                    name,
                    email,
                    feedback,
